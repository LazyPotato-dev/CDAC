...
class DmSimulatorPy_Base(BackendV1):
    ...
    def mpi_unitary_single(self, gate, qubit):
        """Apply an arbitrary 1-qubit unitary transformation.
        ...
        for idx in gate:
            #print(f"before operation {idx}, the state is : {self._densitymatrix1}", type(self._densitymatrix1))
            self._densitymatrix = self._single_rot(
                idx[0],
                idx[1],
                self._error_params["one_qubit_gates"][idx[0]],
                self._densitymatrix,
            ...
            #print(f"after operation {idx}, the state is : {self._densitymatrix1}", type(self._densitymatrix1))
        self._densitymatrix = np.reshape(eval(self._densitymatrix), self._number_of_qubits * [4])
    ...
    def run_experiment(self, experiment):
        ...
        for clock in range(levels):
            ...
            for operation in partitioned_instructions[clock]:
                ...
                if operation.name in ("u1", "u3"):
                    ...
                    # start_unitary_time = time.time()
                    # self._add_unitary_single(gate, qubit)
                    # end_unitary_time = time.time()
                    start_mpi_time = time.time()
                    self.mpi_unitary_single(gate, qubit)
                    end_mpi_time = time.time()
        ...
        if self.SHOW_FINAL_STATE:
            data["Modified_coeffmatrix"] = np.array(eval(self.result)).flatten()
        ...
        return {
            ...
            "shots": 0,
            # "mpi_time_taken": -start_mpi_time + end_mpi_time,
            # "single_unitary_time_taken": -start_unitary_time + end_unitary_time,
        }
    
    def _single_rot(self, gate, param, err_param, state, q, num_qubits):
        file_name = "MPI_PROGRAM.py"
        N = 1 # distance of interval
...
if rank == 0:
    ...
    n_qubits, param, state = {num_qubits}, {param}, {state}
...
if rank == 0:
    ...
    idx = (rank-1)%N
    if rank <= N:
        var = (temp[:, k[0], :][idx::N]) * c - (temp[:, k[1], :][idx::N]) * s
    else:
        var = (temp[:, k[1], :][idx::N]) * c + (temp[:, k[0], :][idx::N]) * s
data = comm.gather(var, root=0)
comm.barrier()
if rank == 0:
    for t in range(N):
        state[:, k[0], :][t::N] = data[t+1]
        state[:, k[1], :][t::N] = data[t+1+N]
    print((state.tolist()))
""")
        #result1 = subprocess.run(f"mpiexec -n {2*N+1} python {file_name}", shell=True, capture_output=True, text=True)
        result1 = subprocess.run(f"RDMAV_FORK_SAFE=1 mpiexec -n {2*N+1} python {file_name}", shell=True, capture_output=True, text=True)
        self.result = (result1.stdout)
        return self.result
